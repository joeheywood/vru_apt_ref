...
r2d3.onRender(function (data, svg, width, height, options) {
  ...
  function ticked() {
    ...
    updateHighlight(options.selectedCategory);
  }

  // Function to update the chart based on the selected borough
  function updateHighlight(selectedBorough) {
    circles.each(function (d) {
      if (d.borough == selectedBorough) {
        // Make sure this matches the actual data property for borough
        d3.select(this).raise().attr("fill", "#EE266D"); // Highlight selected borough in orange #EB861E"
      } else {
        d3.select(this).attr("fill", "#EE266D"); // Set non-selected to the default non-highlighted color
      }
    });
  }
  ...
  circles
    .on("mouseover", function (event, d) {
      ...
      valueText.text(d.value + " in " + d.unique); //+ d.unique_id need this to be the borough.
    })
  ...
  // If the options object has a selectedCategory property, update the highlights
